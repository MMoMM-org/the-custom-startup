...
choose_prompts() {
  ...
  # Default: <docs_base>/multi-ai (relative to project/repo root, not toml_dir)
  local default_base="$SPECS_DIR_NAME/multi-ai"
  ...
  printf "    ${DIM}*.md${RESET}         multi-ai-workflow guide\n"
  printf "\n"
...
_choose_custom_prompts_dir() {
  while true; do
    ...
    # If it's a relative path, it will be created during install — accept it
    if [[ "$PROMPTS_BASE_DIR" != /* ]]; then
      break
    fi

    # Absolute path — check parent exists
    local parent
    parent="$(dirname "$PROMPTS_BASE_DIR")"
    if [[ -d "$parent" ]]; then
      break
    fi

    warn "Parent directory does not exist: $parent"
    printf "  ${CYAN}1)${RESET} Create it now\n"
    printf "  ${CYAN}2)${RESET} Enter a different path\n"
    ask "Select [1-2]:"
    local c
    read -r c </dev/tty
    case "$c" in
      1)
        mkdir -p "$parent" && success "Created: $parent" && break
        ;;
      *)
        continue
        ;;
...
# Download prompt templates, guide, and utility scripts into PROMPTS_BASE_DIR.
# Layout: <base>/templates/*.md, <base>/bin/*.sh, <base>/multi-ai-workflow.md
# Uses local files when running from a clone, remote download otherwise.
_download_prompts() {
  local base="$PROMPTS_BASE_DIR"

  # Resolve relative paths against INSTALL_DIR
  if [[ "$base" != /* ]]; then
    base="$INSTALL_DIR/$base"
  fi

  local templates_dir="$base/templates"
  local bin_dir="$base/bin"

  mkdir -p "$templates_dir" "$bin_dir"
  ...
  # Multi-AI workflow guide — directly in base dir
  if $use_local; then
    cp "$this_dir/docs/guides/multi-ai-workflow.md" "$base/multi-ai-workflow.md" 2>/dev/null \
      && info "Copied: multi-ai-workflow.md" \
      || warn "Could not copy docs/guides/multi-ai-workflow.md"
  else
    curl -fsSL "$REPO_RAW_URL/docs/guides/multi-ai-workflow.md" -o "$base/multi-ai-workflow.md" 2>/dev/null \
      && info "Downloaded: multi-ai-workflow.md" \
