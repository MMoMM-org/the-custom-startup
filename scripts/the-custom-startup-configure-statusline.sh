...
select_location() {
  ...
  echo "  ${CYAN}1)${RESET} Global      — all sessions  (~/.config/the-agentic-startup/ + ~/.claude/settings.json)"
  echo "  ${CYAN}2)${RESET} Current repo — this git repo (.claude/ in detected root)"
  echo "  ${CYAN}3)${RESET} Custom path  — specify a repo path manually"
  echo ""
...
select_plan() {
  ...
  echo "  ${CYAN}4)${RESET} Custom  — enter limit manually"
  echo ""
  ...
  echo "  ${CYAN}1)${RESET} token — tokens used vs plan limit  (recommended for Enhanced)"
  echo "  ${CYAN}2)${RESET} cost  — session USD vs threshold   (standard default)"
  echo ""
