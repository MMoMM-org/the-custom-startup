...
# Supports global (~/.claude) and per-repo (.claude/) installation.
#
# Usage:
...
generate_toml() {
  local toml_path="$1" plan="${2:-pro}" token_limit="${3:-}"

  cat > "$toml_path" << EOF
# The Custom Startup — Enhanced Statusline Configuration
# Generated by configure-statusline.sh — edit freely.
#
# Per-repo override: <repo>/.claude/statusline-enhanced.toml
...
install_enhanced() {
  ...
  if [[ "$SCOPE" == "global" ]]; then
    ...
    local toml_dest="$GLOBAL_CONFIG_DIR/statusline-enhanced.toml"
    ...
    local toml_dest="$repo_root/.claude/statusline-enhanced.toml"
  ...
  if [[ -f "$toml_dest" ]]; then
    if prompt_yn "Config already exists at $toml_dest — overwrite?" "n"; then
      generate_toml "$toml_dest" "$PLAN" "$PLAN_CUSTOM_LIMIT"
    else
