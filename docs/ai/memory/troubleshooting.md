...
<!-- 2026-05-13 -->
Python `strip()` trims only the ends; bash `tr -d '[:space:]'` also removes internal whitespace, so `"h 7"` classified differently in each. → Use `re.sub(r'\s+', '', ...)` to match bash semantics. ADR-4 forbids bash/python divergence.

## spec-014 T1.3: `_write_pr_state_cache` replaces the branch entry rather than merging — Status: resolved
<!-- 2026-05-23 -->
