...
### export-spec.sh — Send a spec to an external AI

Assembles all documents in a spec directory (PRD, SDD, plan phases) into a single portable Markdown file, ready to paste into Claude.ai or Perplexity as context.

```bash
# Interactive — pick a spec from the list
./scripts/export-spec.sh

# Export a specific spec directly
./scripts/export-spec.sh --spec 001-my-feature

# Write to a file instead of stdout
./scripts/export-spec.sh --spec 001-my-feature --output /tmp/spec.md

# Copy straight to clipboard (macOS)
./scripts/export-spec.sh --spec 001-my-feature | pbcopy
```

The output bundles whatever exists in the spec directory: if only the PRD is written so far, only that appears. SDD and plan sections are included automatically once they exist. A header at the top tells the AI tool what the document is and how to import output back.

### import-spec.sh — Bring AI output back into the project

Imports a Markdown file (generated by an external AI) into a spec directory as either `requirements.md` (PRD) or `solution.md` (SDD). Handles spec directory creation and prompts before overwriting existing files.

```bash
# Import a PRD into an existing spec
./scripts/import-spec.sh --type prd --spec 001-my-feature --input prd-output.md

# Create a new spec directory and import the PRD in one step
./scripts/import-spec.sh --type prd --new my-feature --input prd-output.md

# Import an SDD
./scripts/import-spec.sh --type sdd --spec 001-my-feature --input sdd-output.md

# Read from clipboard (macOS)
pbpaste | ./scripts/import-spec.sh --type prd --new my-feature

# Interactive — pick the target spec from a list
