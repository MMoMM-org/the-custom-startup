...
- [x] Create `plugins/tcs-helper/skills/setup/` directory
...
- [x] Implement stack detection (code):
...
- [x] Implement file generation (code):
...
- [x] Implement hook installation (code — calls `merge_hooks.py` from Phase 2):
...
- [x] Optional extras (AI — AskUserQuestion after main generation):
...
- [x] Running setup in a TypeScript repo generates all files with TS-specific content
- [x] Running setup in a repo that already has CLAUDE.md does NOT overwrite it — adds memory section
- [x] All 6 category files are created with starter content
- [x] memory.md index correctly lists all 6 files
- [x] Root CLAUDE.md after setup is < 100 lines
- [x] Running setup twice is idempotent (no duplicate content, no duplicate hooks)
- [x] `cleanupPeriodDays: 99999` is set in `~/.claude/settings.json` after setup
- [x] All 4 hooks (UserPromptSubmit, SessionStart, PreCompact, PostToolUse/Bash) appear in settings.json
- [x] Existing hooks in settings.json are preserved (setup does not overwrite)
