...
| 2026-09-07 | **CRITICAL (T3.4/T3.5): `ms` held SECONDS, not milliseconds — every hook duration was 1000x too small.** Fixed in the writer so the record itself is honest; `ms` is now whole integer milliseconds, and the unit is stated in the SDD rather than implied by the field name | `TIMEFORMAT='%3R'` emits elapsed real time in SECONDS to three decimals. That value went into a field named `ms`, which `report.py` and the README both read as milliseconds. Measured: a `sleep 0.5` recorded `"ms":"0.504"` and rendered as `0.504 ms` — the true 504 ms. This is the one number PRD F6 exists to produce, against CON-7's 1 ms budget: a hook 500x OVER budget reported as comfortably under it, so the instrument built to find slow hooks would have called every hook fast. It survived 30 bats cases, 22 pytest cases, a TDD guardian, and both review gates, because every test wrapped a trivially fast command (`/bin/cat`, `/bin/true`) where 0.001 seconds and 1 millisecond are indistinguishable — nothing asserted a KNOWN duration. Found by wrapping `sleep 0.5` by hand and reading the record. Root cause beyond the code: `solution.md` specified the field only as `ms: number`, with the unit carried by the field NAME alone — so the code matched the spec and spec-compliance review correctly passed it. The SDD now states the unit explicitly, and a bats case pins a known duration so a 1000x error in either direction fails loudly |
| 2026-09-07 | **CON-7 is not met while recording is on**, measured independently on Linux: wrapper with recording off adds ~240 us (within budget); with recording on the hook path costs ~12.2 ms per invocation | 50-iteration measurement: direct `/bin/true` 40 us, wrapped-and-disabled 280 us, wrapped-and-recording 12160 us. The cost is the shared writer path every adapter already pays, not the wrapper — consistent with the ~13.3 ms end-to-end figure this SDD's Quality Requirements already records, and with phase 1's ~9 ms for the writer alone against a 1000 us budget. Recorded here as a numbered decision rather than left in prose, because T3.6's overhead gate has to confront it on macOS and the honest answer today is that the budget holds only when the recorder is switched off |
| 2026-09-07 | **CON-7 is not met on macOS in ANY configuration, and no shell-based hook wrapper can meet it there** (T3.6 overhead gate). Recording off: ~3.0 ms marginal (Linux: ~0.24 ms, within budget). Recording on: ~39.5 ms marginal (Linux: ~12.2 ms). The budget stands as a target for the writer path, but it is now documented as unreachable on macOS rather than presented as a met requirement | Darwin arm64, bash 3.2.57, N=200-300 per configuration, two interleaved passes: direct `/usr/bin/true` ~1.5 ms; wrapper recording OFF ~4.5 ms; wrapper recording ON ~41 ms. Three candidate causes were measured and **two were refuted**, so the remaining number is not a fixable implementation defect: (1) *script size* — stripping the 15019-byte wrapper to 1965 bytes changed nothing (4477 → 4354 us, inside noise); (2) *early-exit position* — hoisting the `ENABLED` check to line 2 made it marginally **worse** (5299 → 6050 us), so bash 3.2 does not short-circuit reading on `exec`. The cost is spawning a shell interpreter at all: a two-line script whose entire body is `exec "$@"` costs ~4.0 ms with `#!/bin/bash` and ~5.1 ms with `#!/usr/bin/env bash`, against a ~1.5 ms direct baseline — so the floor for interposing *any* shell in the macOS hook path is ~2.4 ms, 2.4x CON-7's budget before a single line of our logic runs. macOS charges a code-signature check per exec; Linux does not, which is the whole gap between the two platforms. **One real lever was found and is NOT applied here**: `#!/usr/bin/env bash` → `#!/bin/bash` saves ~1.1 ms per invocation (~22%) by dropping one exec. It is left for a decision because it trades PATH portability for speed across every script in this repo, not just this one, and the Deviation Protocol wants that approved rather than slipped into a validation task |
| 2026-09-08 | **The activation switch moves from the launch environment into a `.claude/settings.json` `env` block, and recording is left ON in this repo.** The shipped documentation previously told the reader to export the variable in a shell profile or type it at launch; that advice is now demoted to "right for a single deliberate measurement, wrong for the question this feature exists to answer" | Measured, not assumed: `CLAUDE_OBSERVABILITY_DATA` was set from `settings.json` alone and the next instruction load wrote its record into that directory rather than the default one, so a `settings.json` `env` value **does** reach a hook the harness itself spawns — the distinction that matters here, because `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` famously do not (see spec 012). The change also applied inside the running session, with no restart. The consequence is what makes this a decision rather than a note: PRD Feature 4 and issue #147 ask which instruction files and skills are *actually used*, and one session answers that for nothing — the byte figures are stable at any sample size, but "82 of 83 never fired" over a single session is not a finding, it is an artefact of the sample. A switch that must be retyped at every launch is a switch that is off, so the data would never have accumulated. Safety is unchanged: recording still requires an explicit act, that act is editing one's own gitignored configuration file, and no plugin update can perform it |
| 2026-09-12 | Implementation complete -- status corrected against git, not its own README | All three phases shipped and merged to `main` in #154, whose title says "phase 1" but whose content is the whole spec: the writer, the three adapters, `selfcheck.sh`, `timed-wrapper.sh` and `report.py` are all present on `main`. Verified by checking each file against `main` rather than by reading this table, which had said `Ready` since 2026-09-07. spec-019 then extended `report.py` from 1630 to 2044 lines and rolled the recorder out to four repositories |
