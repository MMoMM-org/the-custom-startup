...
- [ ] **T3.1 Dispatch detection, executable against every existing spec** `[activity: domain-modeling]`
  ...
  5. Success:
     - [ ] Every existing spec with a plan routes to the incremental loop `[ref: SDD/Quality Requirements; Reliability]`
     - [ ] An unrecognised decomposition artifact stops rather than guessing `[ref: PRD/AC Feature 4.3]`
     - [ ] A recorded-vs-detected mismatch reports before any work is dispatched `[ref: PRD/AC Error Handling 1]`
     - [ ] An absent tier proceeds silently `[ref: PRD/AC Feature 3.3]`

- [ ] **T3.2 `implement-incremental` carries today's loop, unchanged** `[activity: docs]` `[parallel: true]`
  ...
  5. Success:
     - [ ] The phase loop's Workflow text is unchanged `[ref: SDD/ADR-5]`
     - [ ] The skill is hidden from the `/` menu `[ref: PRD/Won't Have]`
     - [ ] The checklist format the loop parses is intact `[ref: SDD/Implementation Boundaries]`

- [ ] **T3.3 `implement-direct` — the phase-less loop** `[activity: docs]` `[parallel: true]`
  ...
  5. Success:
     - [ ] `tdd-guardian` runs before every implementer `[ref: PRD/AC Feature 5.1]`
     - [ ] An explicit approval point exists before work is dispatched `[ref: PRD/AC Feature 5.2]`
     - [ ] The drift check runs on completion `[ref: PRD/AC Feature 5.3]`
     - [ ] The constitution check runs where a CONSTITUTION.md exists, and an L1/L2 violation blocks completion `[ref: PRD/AC Feature 5.4]`
     - [ ] Neither reviewer agent is dispatched `[ref: PRD/AC Feature 5.5]`
     - [ ] Exceeding three delivery units recommends escalation `[ref: PRD/AC Edge Case 1]`
     - [ ] `finalize` is called, so Direct specs do not rot on `Ready` `[ref: SDD/Implementation Gotchas]`

- [ ] **T3.4 `implement` becomes the dispatcher** `[activity: docs]`
  ...
  5. Success:
     - [ ] No plan present routes to `implement-direct` `[ref: PRD/AC Feature 4.1]`
     - [ ] `plan/README.md` present routes to `implement-incremental` `[ref: PRD/AC Feature 4.2]`
     - [ ] The selected route and its trigger are displayed `[ref: PRD/AC Feature 4.4]`
     - [ ] `$ARGUMENTS` reaches the sub-skill unchanged `[ref: PRD/AC Feature 4.5]`
     - [ ] The user-facing entry-point name is unchanged `[ref: SDD/Implementation Boundaries]`
