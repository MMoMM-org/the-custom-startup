...
- [x] **T2.1 Classification rules, executable and traced** `[activity: domain-modeling]`
  ...
  5. Success:
     - [x] The same documents always produce the same recommendation `[ref: PRD/AC Feature 2.3]`
     - [x] The four traced specs classify as the SDD documents `[ref: SDD/Implementation Examples]`
     - [x] Breadth vetoes Direct regardless of change type `[ref: PRD/Edge case Scenario 2]`
     - [x] An unparseable signal degrades to its conservative value rather than blocking `[ref: PRD/AC Edge Case 2]`
     - [x] The reserved Factory branch is present and marked unimplemented, so adding a tier later changes no contract `[ref: PRD/AC Feature 1.3]`

- [x] **T2.2 `xdd` step 6 classifies, confirms, records, and routes** `[activity: docs]`
  ...
  5. Success:
     - [x] Exactly one recommended tier is produced once both documents exist `[ref: PRD/AC Feature 2.1]`
     - [x] The signals that produced it are displayed `[ref: PRD/AC Feature 2.2]`
     - [x] Both tiers are offered, recommendation highlighted, never pre-applied `[ref: PRD/AC Feature 2.4]`
     - [x] Classification costs no user input beyond the confirmation `[ref: PRD/AC Feature 2.6]`
     - [x] A Direct run writes no decomposition artifact `[ref: PRD/AC Feature 1.1]`
     - [x] An Incremental run produces `plan/README.md` and phase files `[ref: PRD/AC Feature 1.2]`
     - [x] Requirements and solution are written at both tiers `[ref: PRD/AC Feature 1.4]`
