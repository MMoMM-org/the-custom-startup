...
- [ ] **T1.1 `spec.py` reports the decomposition tier** `[activity: backend-api]`
  ...
  5. Success:
     - [ ] Reading a pre-tier spec reports an absent tier rather than failing `[ref: PRD/AC Feature 3.3]`
     - [ ] The emitted key name is exactly `decomposition_tier` `[ref: SDD/Data Storage Changes]`
     - [ ] No existing `spec.py` behaviour changed — verified by the pre-existing tests still passing `[ref: SDD/Quality Requirements; Reliability]`

- [ ] **T1.2 Spec README template carries the tier row** `[activity: docs]` `[parallel: true]`
  ...
  5. Success:
     - [ ] A new spec starts with a tier row present and empty `[ref: SDD/Data Storage Changes]`
     - [ ] The row placeholder never reads back as a real tier `[ref: PRD/AC Feature 3.3]`

- [ ] **T1.3 `xdd-meta` exposes tier in its interface and phase transitions** `[activity: docs]`
  ...
  5. Success:
     - [ ] The lifecycle interface carries the tier as a first-class field `[ref: PRD/AC Feature 3.1]`
     - [ ] A Direct spec is never told to "continue to PLAN" `[ref: SDD/Runtime View]`
     - [ ] Recording a tier writes both the Status row and a decision-log row `[ref: PRD/AC Feature 3.2]`
     - [ ] A recorded tier is what explains a missing `plan/` to a later reader `[ref: PRD/AC Feature 1.5]`
