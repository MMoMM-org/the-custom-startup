...
- [x] **T1.1 Plugin Manifest + Hook Registration** `[activity: integration]` `[parallel: true]`
...
- [x] **T1.2 README + CHANGELOG** `[activity: documentation]` `[parallel: true]`

  1. Prime: Read existing `plugins/tcs-helper/README.md` and `CHANGELOG.md`; PRD §Vision + §Value Proposition for messaging.
  2. Test (acceptance criteria, written before prose; verified by `tests/bats/docs_smoke.bats` plus reviewer pass):
     - README contains a one-line reference for each of the 12 PRD Goals (M1–M12) — bats assertion: `grep -c '^- M' plugins/tcs-git-helpers/README.md` ≥ 12.
     - README has sections: Overview, Installation, Basic Usage, References — bats assertion: each `^## ` heading present.
     - All in-document links resolve to existing files — bats assertion: every `](path)` target exists relative to the plugin root.
     - CHANGELOG `## [1.0.0]` entry exists with date — bats assertion: `grep -E '^## \[1\.0\.0\] - 2026' plugins/tcs-git-helpers/CHANGELOG.md` matches.
     - Reviewer (Marcus) confirms English/concise/scope-correct on read-through.
  3. Implement: Create `plugins/tcs-git-helpers/README.md` (overview, installation, basic usage, link to references) and `CHANGELOG.md` (initial v1.0.0 entry summarizing scope per PRD §Goals); also create `tests/bats/docs_smoke.bats` enforcing the criteria above.
  4. Validate: `bats tests/bats/docs_smoke.bats` green; Marcus read-through pass; English, concise, links resolve.
  5. Success: README mentions all 12 Goals at high level `[ref: PRD/§Feature Requirements]`; CHANGELOG follows TCS convention; smoke bats green.

- [x] **T1.3 lib/git_state.sh** `[activity: domain-modeling]` `[parallel: true]`
...
- [x] **T1.4 lib/config_parser.sh** `[activity: domain-modeling]` `[parallel: true]`
...
- [x] **T1.5 lib/pattern_match.sh** `[activity: domain-modeling]` `[parallel: true]`
...
- [x] **T1.6 lib/override.sh** `[activity: domain-modeling]`
...
- [x] **T1.7 lib/cache.sh** `[activity: domain-modeling]` `[parallel: true]`
...
- [x] **T1.8 lib/audit_log.sh** `[activity: domain-modeling]` `[parallel: true]`
...
- [x] **T1.9 Test Fixtures + Synthetic Repos** `[activity: test-fixtures]` `[parallel: true]`
...
- [x] **T1.10 Phase 1 Validation** `[activity: validate]`
