...
├── templates/
│   └── githooks/                                           # NEW (per ADR-2 revised): bundle-versioning home
│       ├── pre-push-rule-enforcer.sh.j2                    # NEW: M6 pre-push template (~40 lines)
│       └── tcs-helper-rule-enforcer-version                # NEW: bundle version marker (single-line)
└── skills/
...
- [x] **ADR-2 (REVISED 2026-05-21): Pre-push hook scaffolding adopts the tcs-git-helpers bundle-versioning pattern** — NOT a standalone snippet
  - **Rationale:** Marcus's feedback on PRD Q10: "we should implement the git-helpers way of versioning here, we all know there will be bugs and changes need to be done.. and having several repos makes it hard to track which repo has the right version if we don't version correctly." Same architecture problem that spec-012 solved for `tcs-git-helpers` applies here: rule-enforcer-produced hooks will have bugs, need updates, and have to propagate across multiple repos that adopt them.
  - **Decision:** Hook templates live in `plugins/tcs-helper/templates/githooks/` (mirroring `plugins/tcs-git-helpers/templates/githooks/`). Each generated hook in a target repo's `.githooks/` ships with a sibling `tcs-helper-rule-enforcer-version` marker file. A drift-check skill (or extension to the existing `tcs-git-helpers` drift mechanism) detects when the installed version is older than the plugin's current template version and prompts the user to re-install. Filename namespace: hooks generated by rule-enforcer use a `rule-enforcer-` prefix or marker comment to coexist with other tcs-git-helpers-bundle hooks.
  - **Trade-offs:** Adds bundle-versioning infrastructure (extra version marker file, extra drift-check path). Pays off the first time a rule-enforcer-produced hook ships a bug — all consumer repos get notified to update, instead of silently running stale code. Cost is one more file per generated hook and one more drift-check codepath.
  - **Implementation note:** PLAN T3.2 must be rewritten to reflect this — pre-push template now lives under `plugins/tcs-helper/templates/githooks/`, bundle-version marker file generation is part of the scaffold step, and the drift-check integration plan is captured as a new Phase 3 task.
  - **User confirmed:** ✅ 2026-05-21 by Marcus (overrode initial standalone direction)
