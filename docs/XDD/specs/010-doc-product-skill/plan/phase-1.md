...
- `[ref: SDD/Building Block View — Directory Map; lines: 244-280]` — directory layout
- `[ref: SDD/Interface Specifications — Skill Frontmatter Contract; lines: 297-320]` — frontmatter
- `[ref: SDD/Interface Specifications — Mode Routing Contract; lines: 322-340]` — mode dispatch
- `[ref: SDD/ADR-1; lines: 765-771]` — mode-router rationale
- `[ref: SDD/ADR-7; lines: 803-808]` — single slash command
- `[ref: PRD/Feature 1-4 user stories; lines: 95-185]` — what each mode must eventually do (this phase only routes; modes implemented in P2-P4)
...
- [ ] **T1.1 Skill Directory Skeleton and Frontmatter** `[activity: build-feature]`
  ...
  1. **Prime**: Read `plugins/tcs-helper/skills/skill-author/SKILL.md` and `plugins/tcs-helper/skills/finish-branch/SKILL.md` for PICS structure reference. Read `~/.claude/rules/authoring.md` for description-quality rules. `[ref: SDD/Implementation Context — Code Context; lines: 117-139]`
  ...
  5. **Success**:
     - [ ] Directory tree matches SDD/Directory Map `[ref: SDD/Directory Map; lines: 244-280]`
     - [ ] Frontmatter is valid and matches SDD §Skill Frontmatter Contract `[ref: SDD/Interface — Skill Frontmatter Contract; lines: 297-320]`
...
- [ ] **T1.2 SKILL.md Mode Router Workflow** `[activity: build-feature]`
  ...
  1. **Prime**: Read SDD §Solution Strategy and §Mode Routing Contract. `[ref: SDD/Solution Strategy; lines: 226-235]` `[ref: SDD/Mode Routing Contract; lines: 322-340]`
  ...
  5. **Success**:
     - [ ] All four pressure scenarios produce documented behaviour `[ref: SDD/Acceptance Criteria — Mode dispatch; lines: 866-869]`
...
- [ ] **T1.3 Mode-File Stubs and Plugin Manifest Bump** `[activity: build-feature]` `[parallel: true]`
  ...
  1. **Prime**: Read `plugins/tcs-helper/plugin.json` to know the current version. Read repo memory feedback `feedback_no-manual-marketplace-sync` (in `~/.claude/projects/.../memory/`).
  ...
  3. **Implement**:
     ...
     - Bump `plugins/tcs-helper/plugin.json` `version` field by minor (3.2.0 → 3.3.0). Do NOT manually copy to cache or marketplace dirs.
  ...
  5. **Success**:
     ...
     - [ ] Skill appears in `/` menu after Claude Code restart `[ref: SDD/Deployment View — Plugin Manifest Update; lines: 731-733]`
