...
| Significant feature work | Spec first with `/tcs-start:specify` |
...
**Example responses**:
- ✅ "This is a significant feature. I'd recommend creating a spec first with `/tcs-start:specify` to ensure we've thought through the edge cases. Alternatively, we could start with the core flow and iterate. Which approach fits your timeline?"
...
1. **Spec first** - Use `/tcs-start:specify` for features that span multiple sessions
