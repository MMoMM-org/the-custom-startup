...
# skills/setup/lib/with_gha.sh — Install the PR-title-check GitHub Actions
...
[ -f "$TEMPLATE_SRC" ] || {
  printf '[tcs-git-helpers:setup] ERROR: GHA template not found: %s\n' "$TEMPLATE_SRC" >&2
  exit 1
}
...
if [ -f "$DST" ] && [ "${TCS_GHA_FORCE:-0}" != "1" ]; then
  ...
  if head -3 "$DST" | grep -q 'tcs-git-helpers'; then
    cp "$TEMPLATE_SRC" "$DST"
    printf '[tcs-git-helpers:setup] Refreshed existing GHA workflow at %s\n' "$DST"
  else
    printf '[tcs-git-helpers:setup] WARN: %s exists and is not from tcs-git-helpers; not overwriting.\n' "$DST" >&2
    printf '[tcs-git-helpers:setup] WARN: Re-run with TCS_GHA_FORCE=1 to overwrite, or remove the file first.\n' >&2
    exit 0
  fi
else
  cp "$TEMPLATE_SRC" "$DST"
  printf '[tcs-git-helpers:setup] Installed GHA workflow at %s\n' "$DST"
fi
