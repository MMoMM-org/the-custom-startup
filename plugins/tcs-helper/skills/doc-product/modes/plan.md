# `plan` Mode — Repo Detection, Skeleton Proposal, and `claude-docs/` Placeholder Writes

**Invocation:** `/doc-product plan`

This mode analyses the current repository to determine its type, selects the matching
skeleton from `templates/`, proposes the resulting `claude-docs/` tree to the author, diffs
against any existing `claude-docs/` pages, requests explicit confirmation, then writes empty
...
- The proposed `claude-docs/` tree (page filenames).
...
- Page path relative to repo root (e.g. `claude-docs/installation.md`)
- Purpose sentence
...
Present the proposed `claude-docs/` tree to the author in a clear, readable format before asking
anything. Example format:

```
Proposed claude-docs/ tree for this obsidian repo:
...
## Step 4: Diff Against Existing `claude-docs/`

Check whether the repo already has a `claude-docs/` directory:

```bash
docs_dir="$REPO_ROOT/docs"
```

If `claude-docs/` does not exist: all proposed pages are new. Skip to Step 5.

If `claude-docs/` exists: for each proposed page, check whether the file already exists.
For each page that already exists, use AskUserQuestion to ask the author for a per-page
choice:

```
claude-docs/installation.md already exists. What would you like to do with it?
...
Here is what plan mode will write:
  ...
  CREATE  claude-docs/installation.md
  KEEP    claude-docs/configuration.md  (existing — not touched)
  REPLACE claude-docs/usage.md          (existing — placeholder replaces content)
  MERGE   claude-docs/usage-advanced.md (existing — not written; you will merge sections manually)
  CREATE  claude-docs/troubleshooting.md
...
**README.md placeholder format:** `claude-docs/README.md` is always part of the proposed skeleton
...
Files written:
  CREATE  claude-docs/README.md
  CREATE  claude-docs/installation.md
  REPLACE claude-docs/usage.md
  CREATE  claude-docs/troubleshooting.md

Files kept (not touched):
  KEEP    claude-docs/configuration.md

Files flagged for manual merge:
  MERGE   claude-docs/usage-advanced.md
...
Next steps:
  ...
  - Run `/doc-product extract` to auto-generate claude-docs/configuration.md from your
    settings source.
...
| Existing `claude-docs/` page | AskUserQuestion per page (Keep / Replace / Merge) |
...
### Example 1: Obsidian plugin — no existing `claude-docs/`
...
No existing claude-docs/ directory found. All pages will be created fresh.

Here is what plan mode will write:

  CREATE  claude-docs/README.md
  CREATE  claude-docs/installation.md
  CREATE  claude-docs/configuration.md
  CREATE  claude-docs/usage.md
  CREATE  claude-docs/troubleshooting.md
  CREATE  claude-docs/settings-reference.md
  CREATE  claude-docs/commands-reference.md

Confirm? (yes / no / edit the list)
```

### Example 2: Python repo — existing `claude-docs/installation.md`

**Invocation:** `/doc-product plan` (repo contains `pyproject.toml`; `claude-docs/installation.md`
already exists)
