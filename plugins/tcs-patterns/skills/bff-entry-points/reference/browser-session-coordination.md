...
- All protected fetches go through the coordinator (a wrapper or interceptor). Realtime connections register a teardown handle with it — `coordinator.track({ close })` returning an untrack function — so authentication loss can reach them. Transport concerns stay with the transport: the SSE/WebSocket wrapper owns its own retry and backoff (`EventSource` retries natively; a WebSocket wrapper implements backoff); the coordinator owns only the authentication interpretation of failures.
- The coordinator also holds the CSRF token (delivered in the login and `current-user` response bodies — see `reference/endpoint-protection.md`) and attaches it as a request header on every mutation. Token rotation arrives with those same responses.
...
- Probe succeeds → the session is fine; leave the failure to the transport's retry and error handling (SSE auto-reconnect, WebSocket backoff). Do not sign the user out because a proxy hiccuped.
...
**Nothing provider-shaped in the browser.** Provider tokens, authorization codes, callback parameters, and IdP state never enter browser state, `localStorage`, or `sessionStorage` — the BFF exists so the browser holds only the HttpOnly cookie it cannot read (the OWASP Session Management Cheat Sheet names BFF as the alternative to browser token storage). The login callback URL is handled server-side; the browser lands on a clean application route. If a callback parameter appears in browser code, the boundary has already failed.

## Browser Test Coverage

Behavior-driven tests (`tcs-patterns:frontend-testing`; mock the BFF at the network boundary):
...
This is the canonical definition of the required browser set. `reference/enforcement-and-testing.md` links here rather than restating it.
