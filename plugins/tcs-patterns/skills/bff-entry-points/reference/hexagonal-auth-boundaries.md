...
Where authentication ends, where authorization lives, and how identity crosses the boundary. This deepens `tcs-patterns:hexagonal`'s rule — authentication is adapter work, authorization is application policy — into an enforceable contract.
...
- **A CLI acting for a human operator** authenticates that operator (device flow, local credential helper — `tcs-patterns:secure-oauth-oidc`) and constructs the same user principal a session would have produced. It gets no shortcut for being "internal".
...
Return refusals as result types, never throws (`tcs-patterns:hexagonal` house rule). The driving adapter translates:
...
Other driving adapters translate the same results into their own vocabulary — a CLI maps refusals to exit codes and stderr, a queue consumer to reject/dead-letter decisions. The results are the contract; the status codes are one adapter's rendering.
...
If this test can only be written through HTTP, authorization lives in the adapter — that is the defect, not a test inconvenience. See `reference/enforcement-and-testing.md` for the full matrix.
