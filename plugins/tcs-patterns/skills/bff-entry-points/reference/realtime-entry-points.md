...
- A non-200 response fails the connection with a generic `error` event — the page cannot see the status code. A 401, a 500, and a wrong content type are indistinguishable to script. This opacity is why the browser coordinator probes `current-user` on stream failure instead of guessing (`reference/browser-session-coordination.md`).
...
- Keep connection-local writer and cursor state with the endpoint; shared fan-out belongs in a workflow module, not in the endpoint leaf.
...
- Idle and absolute session expiry are revocation as far as live connections are concerned: either enforce a maximum connection lifetime no longer than the session's remaining lifetime, or re-validate the session on the heartbeat cadence and close on expiry. The hostile matrix's "expired session → 401" covers the next request; this rule covers the connection that never makes one.
