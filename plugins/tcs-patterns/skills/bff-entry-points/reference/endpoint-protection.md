...
The endpoint contract, the session cookie profile, browser-request policy, and the derived entry catalog. All examples are TypeScript and framework-neutral; `reference/hono-example.md` shows one concrete binding.
...
// the upgrade registrar owns it (reference/realtime-entry-points.md), so the HTTP
// registrar's switch stays exhaustive over HttpEndpointAccess with an assertNever default.
...
The thunk exists exactly when the contract declares a body, for public and protected endpoints alike, and the registrar never installs pre-handler body validation — one uniform rule. A public handler simply awaits `body()` at entry. A protected mutation performs any meaningful coarse authorization available from `{ principal, params }`, then parses the bounded body, authorizes every body-referenced object/field/operation, and only then performs effects.

A direct refusal test uses a spy thunk to prove parsing was skipped when coarse authorization could decide; body-dependent refusal tests instead prove validation and fine authorization completed before every effect.
...
In context-style frameworks (Hono's `(c) =>`) the same properties are delivered through typed context via a thin adapter the registrar owns: `c.var.principal` exists on the context type only for protected contracts. Either way, the load-bearing guarantee is that `AuthenticatedPrincipal` is a branded type whose constructor is exported only by session resolution — an endpoint cannot fabricate one (see `reference/hexagonal-auth-boundaries.md` for the brand), and an import-boundary gate keeps test factories out of production code.

Rules:

- The contract lives at the endpoint leaf — beside its handler and tests — so one review sees the whole story. A path-mirroring layout works well (`endpoints/api/orders/by-order-id/cancel/post.ts`, path parameters as `by-<param>` directories), but the layout is a house choice; the co-location is the rule.
...
- Schemas are real trust-boundary schemas (`tcs-patterns:typescript-strict`): parse, don't assert.
...
- **Opaque, high-entropy ID** (≥128 bits, CSPRNG; ASVS 7.2.3). The server-side session record holds the tokens and user linkage; the cookie value means nothing.
...
**CSRF token.** Session-bound: a synchronizer token for stateful sessions (the default for a BFF, which is stateful by definition), or a signed (HMAC, session-bound) double-submit — the naive double-submit variant is bypassable via cookie tossing (OWASP CSRF Cheat Sheet). A required custom header is an acceptable equivalent for JSON-only APIs behind a strict CORS policy. Layers are cumulative: Origin + Fetch Metadata + content type + token; do not drop the token because `SameSite` exists.
...
Also send on every API response: `X-Content-Type-Options: nosniff`, `Cross-Origin-Resource-Policy: same-origin`, and an accurate `Content-Type` (OWASP HTTP Headers Cheat Sheet).
...
- Error bodies are RFC 9457 problem details with no stack traces, no internal identifiers, no dependency names (`tcs-patterns:api-design` owns the shape).

## The Derived Entry Catalog

Endpoint contracts stay feature-local. Do **not** build a second, hand-maintained central route-security map — it will drift from runtime registration, and the drift is invisible until an incident. Instead, derive a central catalog from the two things that are already true: the feature-local contracts, and the explicit production composition that mounted them.
...
Production composition exports the composed app *and* its catalog. The catalog is derived output — tests consume it, humans read it, nobody edits it. Three reconciliations make it trustworthy (see `reference/enforcement-and-testing.md` for the gates):
...
Dev-only endpoints live in a separate composition that imports the production one, never the reverse; the production catalog test proves their absence (OWASP API9:2023).
