...
Read alongside [OpenID Connect Core 1.0, Errata Set 2](https://openid.net/specs/openid-connect-core-1_0.html), [OpenID Connect Discovery 1.0, Errata Set 2](https://openid.net/specs/openid-connect-discovery-1_0.html), RFC 9700, and RFC 8725. Describes a safe implementation *order*; follow the exact flow or profile section for normative detail.

Order is the point. Most ID Token defects are not a missing check but a check performed after something has already been trusted — a key fetched from a claim that has not been verified, a session created before the nonce comparison. The numbered sequence under "Atomic ID Token validation" is the contribution; treating it as an unordered checklist loses it.
