...
Makes a security review reproducible and actionable. A review is complete only when its scope, evidence, attack paths, requirement sources, remediation, and verification are explicit.

The finding model below is the long form of the `Finding` record in `SKILL.md` — same fields, more guidance on filling each. Two neighbours in TCS:

- `tcs-team:the-architect:review-security` produces PR-scoped security findings. Where its findings touch OAuth or OIDC, they should carry a control ID from `rfc9700-control-catalog.md` so the two can be reconciled rather than double-counted.
- `tcs-workflow:verify` requires command output before a task is called done. The completion gate at the end of this file is the auth-specific form of the same rule: a green checklist without hostile tests is not completion.
