...
Primary standards decide what is required. Blog posts, vendor defaults, and framework examples may explain an implementation; they do not override a protocol BCP or profile.

The most useful section here is the last one. "Claim discipline" is what keeps an audit honest — the difference between "RFC 9700 §2.1.1 requires public clients to use PKCE" and "PKCE enabled" is the difference between a citation and a guess, and only the first survives someone checking it.
