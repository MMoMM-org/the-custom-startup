...
Preserves the requirement strength and applicability of [RFC 9700 / BCP 240](https://www.rfc-editor.org/rfc/rfc9700.html). A navigation and review aid, not a substitute for the RFC: follow the linked section when wording or an exception is disputed, and check the [RFC info/errata page](https://www.rfc-editor.org/info/rfc9700/) before making a high-stakes claim.

The `ID` column is what makes a finding checkable. Cite it in the `control` field of a `Finding` so a reviewer can go straight to the normative text — and so a finding from this skill and one from `tcs-team:the-architect:review-security` can be reconciled instead of double-counted.
