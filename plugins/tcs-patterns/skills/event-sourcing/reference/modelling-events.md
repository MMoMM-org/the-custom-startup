...
Events are the most permanent artefact in an event-sourced system — you will replay them for the life of the system. Time spent modelling them well is repaid many times over. This reference covers discovering events, the command/event distinction, granularity, and the naming anti-patterns.

**Scope note.** Past-tense naming, envelope fields, correlation and causation IDs, and handler idempotency are `tcs-patterns:event-driven`'s rules and apply unchanged here — this reference does not restate them. What follows is what changes when the events are also your *storage*.
...
**Pivotal events** are the few most significant events (Order Placed, Payment Received, Shift Closed) that mark major state transitions. They are the primary signal for **bounded-context and stream boundaries** — they usually mark where one aggregate's lifecycle ends and the next begins. Place them first, then fill in between them. This feeds the short-stream / "closing the books" modelling in `reference/production-concerns.md` directly.
...
## Naming, When the Event Is Also Storage

The general rule is `tcs-patterns:event-driven`'s: past tense, business language. Three additions matter specifically because these events are permanent:

- **Intention-revealing, not technical.** The name carries the business reason. `OrderShippingAddressCorrected` says why; `OrderUpdated` says nothing — and unlike a message, you cannot stop reading it later.
...
- **Greg Young's formulation:** *"All events should be represented as verbs in the past tense such as CustomerRelocated, CargoShipped, or InventoryLossageRecorded."*
...
- **Capture the values that were true at the moment it happened** — the price charged, the tax rate applied, the result returned by an external service. Not a foreign key to a row that will have changed, and not a value the projection must recompute later. Young's rule: bake behaviour-determining data into the event at creation time, so replay stays deterministic even if the algorithm changes.
- **Always include the relevant IDs**, and prefer copying immutable values over references.
- **Model events as discriminated unions** with a `type` discriminant and exhaustive `never`-guarded handling — the same shape as commands and state (`tcs-patterns:typescript-strict`, `tcs-patterns:functional`).

## Granularity: Thin, Fat, and Summary

Event size is a real design decision with named patterns (Mathias Verraes):

- **Thin event** — carries only what the fact needs. The default for internal events. The failure mode is the *clickbait event*: an event carrying only an id, forcing subscribers to call back to the producer, which reintroduces coupling and race conditions.
...
- **Summary event** — a single coarse event concluding a process (`ShiftClosed` with the day's totals), built by a projection over the fine-grained events, for consumers who only care about the outcome.
...
Practically: keep internal events free to change; expose a coarse, enriched, versioned external event (often a *summary* or *fat* event) on a separate channel. This is also what buys you versioning freedom internally (`reference/event-versioning.md`).
