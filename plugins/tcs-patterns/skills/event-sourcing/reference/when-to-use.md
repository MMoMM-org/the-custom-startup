...
| Question | If yes → |
|---|---|
| Is a complete, immutable audit trail / full domain history a hard requirement (finance, health, compliance)? | Strong fit |
| Do you need to reconstruct past state, replay to reproduce bugs, or answer questions nobody anticipated when the data was written? | Strong fit |
| Does the domain have rich behaviour and lifecycle, not just fields being edited? | Candidate |
| Do you need multiple read models over the same facts, added retroactively? | Candidate |
...
**A note on "tamper-evident."** Event sourcing gives a *complete* domain history by construction, but append-only application logic is not by itself tamper-*evidence*: a DBA, a schema migration, or a compromised credential can still alter rows. Provable integrity needs controls on top — hash-chaining the events, WORM/immutable storage, restricted write permissions, external audit logging. Event sourcing makes those easier; it does not supply them for free.
...
3. **Outbox pattern** — durable publication intent saved with the state change, then retried and possibly duplicated delivery attempts. Consumers must be idempotent; permanent failure remains possible and observable. (`tcs-patterns:event-driven`, `reference/event-patterns.md` § Transactional Outbox Pattern.)
4. **Event sourcing** — events *are* the persistence. State is a fold of the log.
...
Adopt with eyes open. You are taking on, permanently:

- **Eventual consistency.** Read models lag writes, and the UI must be designed for it (`reference/projections-and-read-models.md`).
- **Event versioning.** Events are immutable and live forever, so old shapes must stay readable by new code. Decide the strategy on day one (`reference/event-versioning.md`).
...
- **A steeper learning curve** and a smaller pool of people who have done it well.

## Event Sourcing Is Not…

These four confusions cause the most damage. Keep the distinctions crisp.

### …CQRS

**Orthogonal patterns, frequently combined.** CQRS separates the write model from read models; event sourcing chooses events as the write model's storage. You can do CQRS without event sourcing — that is `tcs-patterns:hexagonal`'s **CQRS-lite** (`reference/cqrs-lite.md`): one database, query functions that JOIN freely. In practice event sourcing almost always *drives* CQRS, because the log only answers get-by-id, so events are projected into read models to serve queries. Young's directional rule of thumb — *"You can use CQRS without Event Sourcing, but with Event Sourcing you must use CQRS"* — is practical advice, not a formal law; the patterns are independent, and Fowler considers CQRS optional and warns it *"adds risky complexity"*.

### …event-driven architecture

*"Event-driven"* is an overloaded umbrella. Fowler names four different patterns hiding under it: **event notification**, **event-carried state transfer**, **event sourcing**, and **CQRS**. Notification and carried-state-transfer are about *messaging between systems*; event sourcing is about *persistence within one system*. Publishing an event to tell another service something happened is event-driven and needs no event store — that is `tcs-patterns:event-driven`'s territory. Only storing events as your source of truth is event sourcing.
...
The difference is the **direction of truth**. In event sourcing, events are primary and state is derived. In change-data-capture or an audit log, database *state* is primary and the log is a derived side effect. A 100%-reliable audit log is a *free benefit* of event sourcing, not the point — and the reverse, treating a CDC row-change stream as your domain event stream, couples every consumer to your physical schema. Kislay Verma: *"some people propose to use the CDC stream as a system's event stream, and this is where I completely disagree."*

## If You Decide Against It

That is the common, correct outcome. Reach instead for current-state storage with the patterns in `tcs-patterns:ddd` and `tcs-patterns:hexagonal`; an append-only history table for audit; the **outbox pattern** for reliable cross-service events (`tcs-patterns:event-driven`); and **CQRS-lite** for read/write separation (`tcs-patterns:hexagonal`, `reference/cqrs-lite.md`).

You can adopt event sourcing later for one context without rewriting the rest — the Decider you already wrote is the migration path.
