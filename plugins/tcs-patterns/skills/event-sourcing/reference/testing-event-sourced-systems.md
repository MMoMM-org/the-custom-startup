...
Event sourcing is one of the most testable patterns there is: the write model is pure data in and pure data out. This reference shows how to test deciders, projections, and upcasters as **behaviour through the public API** — `tcs-patterns:testing`'s approach — and how that relates to the "given-when-then" idiom the event-sourcing literature reaches for.
...
Follow the repository's established test style. A direct behaviour-driven expression follows `tcs-patterns:testing`: call the public function, assert on the observed output, name the test after the business behaviour, and build data with factory functions. An existing given/when/then helper may encode the same algebra — but do not add a new DSL only for event-sourced code. The direct form needs no extra machinery, because the three moves are already plain function calls on plain data:

| The literature's idiom | Direct behaviour-driven expression |
|---|---|
| *given* past events | Build the starting state — fold event factories with `evolve`, or use a state factory. Past events are just data |
...
No event bus and no mocks are required.

## Testing the Decider

`decide` returns its events (or a rejection) as a value, so asserting on that return value is behaviour testing through the public API. Build the prior state by folding factory-built events with `evolve`. (The account decider under test is in `reference/decider-and-rehydration.md`.)

```typescript
// Event factories — complete, valid data with overrides (the testing skill's factory pattern)
...
The starting state is a fold of factory events, the action is a direct call, and the assertion is on the returned data. Test names describe business behaviour, not mechanics.

Use factories when histories repeat or read more clearly that way; one-off inline events are fine when explicitly typed. An *uncontextualized* array literal may widen its `type` discriminant to `string` — annotate it as `AccountEvent[]` or use `satisfies readonly AccountEvent[]`. Cover the branches that matter: `tcs-patterns:mutation-testing`'s mutator awareness applies, so avoid identity values like a withdrawal of exactly the balance unless the boundary *is* the behaviour under test.

## Testing `evolve` Through Behaviour, Not Directly

Do not write a 1:1 `evolve.test.ts` asserting each transition in isolation — that tests an implementation detail (`tcs-patterns:testing`'s "no 1:1 mapping" rule). `evolve` is exercised thoroughly by the decider tests above, every one of which folds events through it. If you want to pin down rehydration as a behaviour, assert on what the folded state *lets you do*:
...
The command handler is impure — it touches the store — so test it against an **in-memory `EventStore` fake**: a real implementation of the port backed by a `Map`, not a mock (`tcs-patterns:hexagonal`'s fakes-not-mocks rule). This proves load → rehydrate → decide → append works end to end, including optimistic concurrency.
...
**Then prove idempotency with a redelivery**, because at-least-once delivery permits duplicate attempts and production consumers must tolerate them. An idempotent projector no-ops on an event it has already applied — assert the balance does not double-count.

The projector row below is **scoped to one account (one stream)**, so the stream `version` is a valid dedupe key. A projection folding across many streams would instead key on the **event id** or **global position**, because versions repeat across streams:
...
- **Business invariants** — after any sequence of accepted deposits and withdrawals, the balance never goes negative, because `decide` rejects overdrafts.
