...
- Exporter endpoint and headers come from standard env vars (`OTEL_EXPORTER_OTLP_ENDPOINT`, etc.) — config via environment, consistent with `tcs-patterns:twelve-factor`
...
Wire this into the logger factory once (pino mixin, winston format, or the custom logger from `tcs-patterns:twelve-factor`) rather than at call sites. Several ecosystems ship this ready-made (e.g. pino's OTel transport); prefer the built-in integration when one exists.
