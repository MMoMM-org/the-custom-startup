...
Instrumentation is behavior: someone (an operator, a dashboard, an SLO) depends on it, so it is test-driven like any behavior (see `tcs-workflow:xdd-tdd` and `tcs-patterns:testing`). Untested instrumentation is the classic silent regression — nothing fails when a refactor drops the one attribute an alert was built on.
...
Assert through the public API — drive the handler, inspect the exported spans. Do not spy on `span.setAttributes` calls; that tests HOW, not WHAT (see `tcs-patterns:testing`).

## Testing the Canonical Wide Event

The wide event is a contract with operations: test it like one. Inject a recording logger (`tcs-patterns:twelve-factor`'s logger semantics make this a plain fake) and assert the emitted event's fields:
...
Where domain-significant observations flow through an explicit driven port (a Domain Probe — see the four-tier model in `tcs-patterns:hexagonal`), the port gets a recording fake like every other driven port. The worked example lives in `tcs-patterns:hexagonal` (`reference/testing-hex-arch.md`); the shape is:
...
Alert rules are code and deserve tests, but the rules themselves belong to
`tcs-team:the-devops:monitor-production`. Test them where they are defined.
